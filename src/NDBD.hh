...
#define NDBD_R_RB    1e+4   // when using QAG, we integrate over [RA,RB] ~= [0,Inf), so make RB [fm] reasonably large (typical choice = ????)
...
#define NDBD_Q_QB    2.5e+3 // when using QAG, we integrate over [QA,QB] ~= [0,Inf), so make QB [MeV] reasonably large (typical choice = ????)
